...
class HelperService extends Service {
	...
	public static function sendEmails($sSubject,$sText) {
		$aEmails = ConfigService::getItem('alert_to_emails');
		$sFromEmail = ConfigService::getItem('support_email');
		foreach ($aEmails as $sEmail) {
			self::sendEmail($sEmail,$sFromEmail,$sSubject,$sText,str_replace("\n","<br />\n",$sText));
		}
	 ...
	 */
	public static function sendEmail($sTo,$sFrom,$sSubject,$sText,$sHtml,$sBCC=false) {
		$oClient = new Postmark\PostmarkClient(ConfigService::getItem('postmarktoken'),60);
		
		if (!$sBCC) {
		    $sBCC = $_ENV['TEAM_EMAIL'];
		}
		
		try {
			$oResponse = $oClient->sendEmail(
				$sFrom
				,$sTo
				,$sSubject
				,$sHtml
			...
			log_message('error','sending email to '.$sTo.' exception: '.$e->getMessage());
	 ...
	 * @param string $sText
	 * @param string $sHtml
	 * @param bool|string $sBCC
	 * @return ServiceResponse
	 */
	public function sendTransactionalEmail($sTo,$sFrom,$sSubject,$sText,$sHtml,$sBCC=false) {
		return false;
		$oEmail = $this->_getLibrary('email');

		//$aMailSettings = ConfigService::getItem('external_smtp_mix');
		//$aConfig = array_shift($aMailSettings);
		//$oEmail->updateConfig($aConfig);
		$oEmail->protocol = 'sendmail';
		$oEmail->clear();
		$oEmail->to($sTo);
		...
		if (!empty($sHtml)) {
			$oEmail->message($sHtml);
			$oEmail->set_alt_message($sText);
		} else {
			$oEmail->message($sText);
		}

		$bResult = $oEmail->send();

		if (!$bResult) {
			foreach ($aMailSettings as $iIndex=>$aConfig) {
				$mError = $oEmail->print_debugger();
				log_message('error','sendTransactionalEmail: email send error: '.print_r($mError,true));

				// 2 seconds = 2000000
				usleep(500000);

				log_message('error','sendTransactionalEmail: trying next config');
				$oEmail->updateConfig($aConfig);
				$bResult = $oEmail->send();

				if ($bResult) {
					log_message('required','sendTransactionalEmail: success retry -> '.$iIndex);
			...
			if (!$bResult) {
				$mError = $oEmail->print_debugger();
				log_message('error','sendTransactionalEmail: final email send error: '.print_r($mError,true));

				return $this->_setupErrorResponse(
					ServiceResponse::StatusBadRequest
					,'Unable to send email.  Please contact support.'
				);
			}
		}

		log_message('required','sendTransactionalEmail: success fall-through');
		return new ServiceResponse();
	}

	/**
	 * Is safe to echo in template
	 *
	 * @static
	 * @access public
	 ...
	 * @return boolean
	 */
	public static function isOutputSafe($sString) {
		if (!is_string($sString)) {
			return false;
		}

		$sMinusTags = strip_tags($sString);
		if (strcmp($sString,$sMinusTags)!==0) {
			return false;
		}
	 ...
	 * @return string|array
	 */
	public function getUrlContents($sUrl,$bReturnHeaders=false) {
		if (!empty($_SESSION['debug'])) {
			log_message('required','Start curl: '.$sUrl);
		}

		$oCurlConnection = $this->_initCurl();
	 ...
	 * @return array
	 */
	public function postUrlContents($sUrl,$aPostData=array(),$iTimeout=90) {
		$oCurlConnection = $this->_initCurl();
		$oCurlConnection = $this->_setUserAgent($oCurlConnection);
