...
abstract class AuthorizeNetRequest
{
    ...
    public $VERIFY_PEER = true; // attempt trust validation of SSL certificates when establishing secure connections. Edited
    ...
    protected function _sendRequest()
    {
		...
		log_message('required','$post_url: '.$post_url);
		log_message('required','_post_string: '.print_r($this->_post_string,true));
        ...
        if ($this->VERIFY_PEER) {
            curl_setopt($curl_request, CURLOPT_CAINFO, APPPATH.'libraries/shared/ssl/cert.pem');
        } else {
			if ($this->_log_file) {
    	...
    	*/
        ...
        if ($this->_log_file) {
            ...
            if ($curl_error = curl_error($curl_request)) {
                file_put_contents($this->_log_file, date('Y-m-d H:i:s')." ----CURL ERROR----\n$curl_error\n\n", FILE_APPEND);
            }
            // Do not log requests that could contain CC info.
             file_put_contents($this->_log_file, date('Y-m-d H:i:s')." ----Request----\n{$this->_post_string}\n", FILE_APPEND);

            file_put_contents($this->_log_file, date('Y-m-d H:i:s')." ----Response----\n$response\n\n", FILE_APPEND);
        }
