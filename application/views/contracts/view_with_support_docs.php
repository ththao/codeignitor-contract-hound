							...
							<table>
								...
								<tr data-cfid="<?php echo $oCustomField->custom_field_id; ?>">
									...
									<td class="form-response" colspan="3">
										...
										<?php if ($oCustomField->type==CustomFieldModel::TYPE_MULTILINE): ?>
										<div ng-show="!contract_editing" class="form-control-static form-control-static-multiline" placeholder="none..."><?php echo wordwrap(retud($mValue),45,'<br/>',true); ?></div>
										<?php elseif ($oCustomField->type==CustomFieldModel::TYPE_TEXT): ?>
										<div ng-show="!contract_editing" class="form-control-static" placeholder="none..."><?php echo wordwrap(retud($mValue),45,'<br/>',true); ?></div>
										<?php else: ?>
										<div ng-show="!contract_editing" class="form-control-static" placeholder="no"><?php echo ($mValue?'yes':'no'); ?></div>
										<?php endif; ?>
									</td>
								</tr>
								<?php endforeach; ?>
					...
					<div class="btn-group">
						<div class="dropdown">
							...
							<ul class="dropdown-menu dropdown-menu-right dropdown-menu-large">
								...
								<!-- <li><a href="#" data-toggle="modal" data-target="#support_docs">Supporting Documents</a></li> -->
								<li class="divider"></li>
						...
						<table class="table table-borderless table-justified">
							...
							<?php foreach ($oRevisions as $oRevision): ?>
								<tr>
									...
									<td><span data-toggle="tooltip" title="<?php //echo date('m/d/Y \a\t g:ia T', strtotime($oRevision->revision_date)); ?><?php echo convertto_local_datetime($oRevision->revision_date,$time_zone,'%x at %I:%M%p %z'); ?>
"><?php //echo time_ago($oRevision->revision_date); ?><?php echo time_ago(convertto_local_datetime($oRevision->revision_date,$time_zone,'%x %X')); ?></span></td>
								</tr>
							<?php endforeach; ?>
							</tbody>
						</table>
