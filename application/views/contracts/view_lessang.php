<style>
  ...
  #contract-details-show .form-control-static {
      height: auto;
  }
</style>
		...
		<div class="layout-panel-main">
			<div class="layout-section">
				<div class="layout-panel-left">
					...
					<div class="divider">
						<div class="divider-content">
							...
							<div class="divider-actions">
								<a id="toggle-edit" href="#" class="btn btn-default btn-sm">edit</a>
							</div>
							<?php endif; ?>
						</div>
					</div>

					<div id="contract-details-show" class="details">
						<div class="form-grid form-minimal">
							<table>
								<tr>
									<td class="form-label"><label>Company</label></td>
									<td colspan="3" class="form-response">
										<div class="form-control-static"><?php if ($oContract->company) { echud($oContract->company); } else { echo 'none...'; } ?></div>
									</td>
								</tr>
								<tr>
									<td class="form-label"><label>Start</label></td>
									<td class="form-response">
										<div class="form-control-static"><?php if ($oContract->start_date) { echo convertto_local_datetime($oContract->start_date,$time_zone);/*echo date('n/j/Y',strtotime($oContract->start_date)); */ } else { echo 'none...'; } ?></div>
									</td>
									<td class="form-label"><label>End</label></td>
									<td class="form-response">
										<div class="form-control-static"><?php if ($oContract->end_date) { echo convertto_local_datetime($oContract->end_date,$time_zone); /*echo date('n/j/Y',strtotime($oContract->end_date));*/ } else { echo 'none...'; } ?></div>
									</td>
								</tr>
								<tr>
									<td class="form-label"><label>Value</label></td>
									<td class="form-response">
										<div class="form-control-static"><?php if ($oContract->valued !== null) { echo $sCurrency.number_format($oContract->valued); } else { echo 'none...'; } ?></div>
									</td>
									<td class="form-label"><label>Type</label></td>
									<td class="form-response">
										<div class="form-control-static"><?php echo ($oContract->type?tl('buy-side'):tl('sell-side')); ?></div>
									</td>
								</tr>
								<tr>
								...
								<?php if (isset($oBoard)): ?>
								<tr>
									<td class="form-label"><label>Folder</label></td>
									<td colspan="3" class="form-response"><p class="form-control-static"><a href="/boards/view/<?php echo $oBoard->board_id; ?>"><em><?php echo $oBoard->name; ?></em></a></p></td>
								</tr>
								<?php endif; ?>
								...
								<tr data-cfid="<?php echo $oCustomField->custom_field_id; ?>">
									...
									<td class="form-response" colspan="3">
										...
										<div class="form-control-static form-control-static-multiline" placeholder="none..."><?php echo wordwrap(retud($mValue),45,'<br/>',true); ?></div>
										<?php elseif ($oCustomField->type==CustomFieldModel::TYPE_TEXT): ?>
										<div class="form-control-static" placeholder="none..."><?php echo wordwrap(retud($mValue),45,'<br/>',true); ?></div>
										<?php else: ?>
										<div class="form-control-static" placeholder="no"><?php echo ($mValue?'yes':'no'); ?></div>
										<?php endif; ?>
									</td>
								</tr>
								<?php endforeach; ?>
					...
					<form id="contract-details-edit" class="details" style="display:none;" method="post" action="/contracts/edit/<?php echo $oContract->contract_id; /* ng-submit="contract_editing=false"*/?>">
						<div class="form-grid form-minimal">
							<table>
								<tr class="ng-hide">
									<td class="form-label"><label>Name</label></td>
									<td colspan="3" class="form-response">
										<input name="name" class="form-control" type="text" placeholder="contract title..." ng-model="contract_name" id="contract_name" />
									</td>
								</tr>
								<tr>
									<td class="form-label"><label>Company</label></td>
									<td colspan="3" class="form-response">
										<input name="company" class="form-control" type="text" placeholder="company name..." ng-model="contract_company" />
									</td>
								</tr>
								<tr>
									<td class="form-label"><label>Start</label></td>
									<td class="form-response">
										<input name="start_date" class="form-control" type="text" placeholder="" data-jquery="datepicker" ng-model="contract_start" />
									</td>
									<td class="form-label"><label>End</label></td>
									<td class="form-response">
										<input name="end_date" class="form-control" type="text" placeholder="" data-jquery="datepicker" ng-model="contract_end" />
									</td>
								</tr>
								<tr>
									<td class="form-label"><label>Value</label></td>
									<td class="form-response">
										<input name="valued" class="form-control" type="text" placeholder="10000..." ng-model="contract_value" />
									</td>
									<td class="form-label"><label>Type *</label></td>
									<td class="form-response">
								...
								<tr data-cfid="<?php echo $oCustomField->custom_field_id; ?>">
									<td class="form-label"><label><span><?php echo str_replace("\n",'<br/>',word_wrap(retud($oCustomField->label_text . ($oCustomField->required ? ' *' : '')),35,'<br/>',true)); ?></span></label></td>
									<td class="form-response" colspan="3">
										<?php if ($oCustomField->type==CustomFieldModel::TYPE_MULTILINE): ?>
										...
										<div class="form-group">
											<label class="option">
												<input type="checkbox" id="cf_<?php echo $oCustomField->custom_field_id; ?>" name="cf[<?php echo $oCustomField->custom_field_id; ?>]" value="1" <?php echo $mValue ? 'checked' : ''; ?>/>
						...
						<div>
							...
							<a class="btn btn-text" id="form-cancel">Cancel</a>
						</div>
					</form>
					...
					<div class="divider">
						<div class="divider-content">
							...
							<div class="divider-actions">
								...
								<?php if (!empty($aApprovalStepsSorted) || (!empty($oSignatures) && $oSignatures->count)): ?>
								<a class="btn btn-default btn-sm" href="#approval-edit-warning" data-toggle="modal">
								<?php else: ?>
								<a class="btn btn-default btn-sm" href="/contracts/update_team/<?php echo $oContract->contract_id; ?>">
								<?php endif; ?>
					...
					<div class="members" id="members-view">
						...
						<a href="/users/profile<?php if ($iCurrentlyLoggedInMemberId == $iCurrentlyLoggedInParentId): ?>_admin<?php endif; ?>/<?php echo $oMember->member_id; ?>" class="member" id="member-<?php echo $oMember->member_id; ?>">
					...
					<div class="divider divider">
						<div class="divider-content">
							...
							<div class="divider-actions">
								...
								<?php endif; ?>
								<a href="/reminders/add/<?php echo $oContract->contract_id; ?>" class="btn btn-default btn-sm pull-center">
									<span data-icon="add-small"></span>
								</a>
								<?php endif; ?>
							</div>
					...
					<?php if (!$oReminders->count): ?>
						<a href="/reminders/add/<?php echo $oContract->contract_id; ?>" class="btn btn-primary float-center">Add a Reminder</a>
					<?php endif; ?>

					<div class="reminders" ng-hide="contract_reminders_editing">
						<?php $iCurrentTime = time(); $i5Days = strtotime('+5 days'); $i21Days = strtotime('+21 days'); 
						...
						<div class="reminder reminder-<?php if ($iAlertDate < $i5Days): ?>danger<?php elseif ($iAlertDate < $i21Days): ?>warning<?php else: ?>default<?php endif; ?>">
							<div class="reminder-content">
								<a href="/contracts/view/<?php echo $oReminder->contract_id; ?>" class="reminder-time"><?php echo convertto_local_datetime($oReminder->alert_date,$time_zone,'%x'); ?></a>
					...
					<div class="reminders" ng-show="contract_reminders_editing">
						...
						<div class="reminder reminder-default">
							<div class="reminder-content">
								<div class="reminder-time"><?php echo convertto_local_datetime($oReminder->alert_date,$time_zone); ?></div>
										...
										<div class="message-content">
											...
											<div class="message-body">
												...
												<div class="message-comment">
													<blockquote><?php echo link_urls(retud($oLog->message)); ?></blockquote>
												</div>
											</div>
										</div>
									</div>
					...
					<div class="btn-group">
						<div class="dropdown">
							...
							<ul class="dropdown-menu dropdown-menu-right dropdown-menu-large">
								...
								<!-- <li style="display: none;"><a href="#" data-toggle="modal" data-target="#support_docs">Supporting Documents</a></li> -->
								<li class="divider"></li>
	...
	<div class="modal-container">
		<div class="modal-dialog">
			<div class="modal-content">
						...
						<table class="table table-borderless table-justified">
							...
							<?php foreach ($oRevisions as $oRevision): ?>
								<tr>
									...
									<td><span data-toggle="tooltip" title="<?php // date('m/d/Y \a\t g:ia T', strtotime($oRevision->revision_date)); ?><?php echo convertto_local_datetime($oRevision->revision_date,$time_zone,'%x at %I:%M%p %z'); ?>"><?php //echo time_ago($oRevision->revision_date); ?><?php echo time_ago(convertto_local_datetime($oRevision->revision_date,$time_zone,'%x %X')); ?></span></td>
								</tr>
							<?php endforeach; ?>
							</tbody>
						</table>
				...
				<div class="modal-footer modal-footer-left">&nbsp;</div>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		scope.$apply(function() {
			...
			scope.contract_value = '<?php if ($oContract->valued) { echo $sCurrency.number_format($oContract->valued); } ?>';
	...
	$('#toggle-edit').click(function(e) {
		...
		$(this).hide();
		$('#contract-details-show').hide();
		$('#contract-details-edit').show();
	});

	$('#form-cancel').click(function() {
		$('#toggle-edit').show();
		$('#contract-details-edit').hide();
		$('#contract-details-show').show();
	});
