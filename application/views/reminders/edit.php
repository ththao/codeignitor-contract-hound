...
<html dir="ltr" lang="en-US">
	<head>
		...
		<title>Edit Reminder | Contract Hound</title>
	...
	<body ng-app="ContractHoundApp">
		<div class="modal fade" id="reminder-modal">
			<div class="modal-container">
				<div class="modal-dialog">
					<form action="/reminders/edit/<?php echo $oReminder->reminder_id; ?>" method="post">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span data-icon="close-small">Close</span></button>
							<h3 class="modal-title">Edit Reminder<?php if (!empty($oContract)):?> <small><?php echud($oContract->name); ?></small><?php endif; ?></h3>
						</div>
						<div class="modal-body">
							...
							<p class="text-large text-light">
								...
								<a id="toggle-show-team-members" href="#" ng-click="selecting_members=true">
									team members
								</a>
								on
								<input class="form-control input-lg input-link" size="12" type="text" id="reminder-date" name="alert_date" value="<?php
								echo convertto_local_datetime($oReminder->alert_date,$time_zone); ?>" data-jquery="datepicker" />
							</p>
								...
								<div class="divider divider-flush">
									<div class="divider-content">
										<div class="divider-title">
											...
											<small>(<?php echo count($aReminderMembers); ?> selected)</small>
										</div>
								...
								<div class="members">
									<?php foreach ($aTeamMembers as $oMember): ?>
										<label class="member member-option">
											<input name="reminder_members[]" type="checkbox"
												<?php if (in_array($oMember->member_id,$aReminderMembers)): ?>class="was-checked" checked="checked"<?php endif;
												?> value="<?php echo $oMember->member_id; ?>" />
							...
							<textarea name="message" class="form-control input-lg" placeholder="Type a descriptive reminder with instructions..."><?php echud($oReminder->message); ?></textarea>
						</div>
		...
		<script>
			...
			$('#select-cancel').click(function() {
				...
				$('input[type=checkbox][class=was-checked]').prop('checked', true);
				$('.divider-title small').text('( '+$('input[type=checkbox]:checked').length+' SELECTED)');
				return false;
			});
